...
fn map_pixel(
    pixel: usize,
    pixel_count: usize,
    image_lower_bound: Precision,
    image_upper_bound: Precision,
) -> Precision {
    (image_upper_bound - image_lower_bound) / pixel_count as Precision * pixel as Precision
        + image_lower_bound
}
...
pub fn mandelbrot(
    pixel_height: usize,
    pixel_width: usize,
    complex_lower: Precision,
    complex_higher: Precision,
    complex_left: Precision,
    complex_right: Precision,
) -> Vec<Vec<Iterations>> {
    (0..pixel_height)
        .into_par_iter()
        .map(|i| {
            ...
            (0..pixel_width)
                .map(|j| {
                    ...
                    while z_real_squared + z_imag_squared < Precision::from(4)
                        && iter < MAX_ITERATIONS
                    {
